// duffing.c, https://github.com/bhanson10/gbees/tree/main/examples/duffing
...
void duffing(double* f, double* x, double t, double* coef){
    f[0] = x[1];
    f[1] = x[0] - x[0] * x[0] * x[0] - coef[0] * x[1] + coef[1] * cos(coef[2] * t); 
}
...
int main(void){
    ...
    double coef[] = {0.35, 0.3, 1};                           // duffing trajectory attributes (a, b, w)
    Traj T = Traj_create(3, coef);                            // Inputs: (# of coefficients, coefficients)

    int NUM_DIST = 11;                                        // Number of distributions recorded per measurement
    ...
    bool TV = true;                                           // Time-invariant dynamics 
    bool BINARY = false;                                      // Binary output file
    //==========================================================================================================//

    //================================================= GBEES ==================================================//
    run_gbees(duffing, NULL, NULL, G, M, T, P_DIR, M_DIR, NUM_DIST, NUM_MEAS, DEL_STEP, OUTPUT_FREQ, CAPACITY, DIM_f, OUTPUT, RECORD, MEASURE, BOUNDS, COLLISIONS, TV, BINARY);
